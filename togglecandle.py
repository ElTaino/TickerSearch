...
# --- REVISED SECTION: TOP TICKER BANNER WITH PAUSE TOGGLE ---
try:
    # 1. Provide high-volume tracking symbols
    active_tickers = ["SPY", "QQQ", "NVDA", "AAPL", "TSLA", "MSFT", "AMD", "AMZN"]
    
    # 2. Fetch daily snapshots
    ...
    # Create a layout with two columns: a wide track for the ticker tape and a tight panel for the button
    banner_col, toggle_col = st.columns([0.88, 0.12])
    
    with toggle_col:
        # 3. Streamlit boolean switch to change playback state variables
        pause_animation = st.checkbox("⏸️ Pause Tape", value=False)
        # Checkbox maps to active CSS states
        play_state = "paused" if pause_animation else "running"

    with banner_col:
        # Duplicate the loop sequence to make the moving track seamless without breaking gaps
        marquee_content = " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ".join(ticker_items)
        double_content = f"{marquee_content} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {marquee_content}"
        
        # 4. Inject a highly responsive CSS animation frame that references our 'play_state' variable
        st.markdown(f"""
            <style>
                ...
                .ticker-move {{
                    ...
                    animation-play-state: {play_state}; /* 🌟 MAPS TO STREAMLIT STATE VARIABLE */
                    color: #ffffff;
                    font-family: monospace;
                    font-size: 15px;
                }}
                .ticker-move:hover {{
                    animation-play-state: paused; /* Automatically pause if user hovers mouse over values */
                }}
            </style>
...
if ticker_symbol:
    try:
        ...
        if stock_data.empty:
            ...
            # --- PANEL 1: Main Price Chart + MA Overlays + Bollinger Bands ---
            # 🌟 RESPONSIVE SWITCH: Dynamically toggle chart styles based on sidebar selection
            if chart_type == "Line":
